...
1. **Inter-chromosomal Parallel Chaining Algorithm**
2. **Intra-chromosomal Parallel Chaining Algorithm**
3. **Inter-chromosomal Parallel Backtracking Algorithm**
...
6. **SIMD DP Chaining** (from [Intel TAL](https://github.com/IntelLabs/Trans-Omics-Acceleration-Library))
...
3. **Jemalloc** - [Jemalloc](https://github.com/jemalloc/jemalloc) (optional)
...
### Accuracy evaluation
The accuracy of mm2-opt can be verified against minimap2 (v2.28). Ensure that the max-chain-skip=infinity parameter is used during the chaining phase to ensure high precision. Follow these steps to verify accuracy:
```bash
# run mm2-opt
...
```
The output generated by mm2-opt and minimap2 should match. 
```bash
# get the difference
diff mm2.paf mm2-opt.paf
```
The above ```diff``` command should return null ouput, i.e. 0 lines.

### Performance
We observed up to a 7.6x speedup in genome-to-genome alignment across all tested datasets, as detailed in our [paper](#citation). The speedup for long-read alignment is comparable to that achieved by [mm2-fast](https://github.com/bwa-mem2/mm2-fast). For a thorough evaluation and detailed experimentation, please refer to our [paper](#citation).
