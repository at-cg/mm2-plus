/*************************************************************************************
Used from BWA-MEM2 under MIT license, with modifications to support SIMD dispatching for multiple instruction set architectures (ISAs).
*****************************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char *argv[])
{
	...
	strcpy(prefix, buf);
	strcat(prefix, &argv0[base_st]);
	//prefix_len = strlen(prefix);
	simd = x86_simd();
	if (simd & SIMD_AVX512BW) test_and_launch(argv, prefix, ".avx512");
	if (simd & SIMD_AVX2) test_and_launch(argv, prefix, ".avx2");
	if (simd & SIMD_AVX) test_and_launch(argv, prefix, ".avx");
	if (simd & SIMD_SSE4_2) test_and_launch(argv, prefix, ".sse4.2");
	if (simd & SIMD_SSE4_1) test_and_launch(argv, prefix, ".sse4.1");
	free(prefix);
