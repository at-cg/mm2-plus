...
static inline mm_reg1_t *mm_map_aux(const mm_idx_t *mi, const char* seqname, const char *seq1, const char *seq2, int *n_regs, mm_tbuf_t *b, const mm_mapopt_t *opt)
{
	...
	if (seq2 == 0) {
		...
		mm_map_frag(mi, 2, len, (const char**)seq, _n_regs, regs, b, opt, seqname, 1);
...
static inline char *mappluspy_revcomp(int len, const uint8_t *seq)
{
	int i;
	char *rev;
...
static char *mappluspy_fetch_seq(const mm_idx_t *mi, const char *name, int st, int en, int *len)
{
	int i, rid;
	char *s;
	*len = 0;
...
static mm_idx_t *mappluspy_idx_seq(int w, int k, int is_hpc, int bucket_bits, const char *seq, int len)
{
