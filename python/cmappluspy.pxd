...
cdef extern from "cmappluspy.h":
	...
	char *mappluspy_fetch_seq(const mm_idx_t *mi, const char *name, int st, int en, int *l)
	mm_idx_t *mappluspy_idx_seq(int w, int k, int is_hpc, int bucket_bits, const char *seq, int l)
	...
	char *mappluspy_revcomp(int l, const uint8_t *seq)
